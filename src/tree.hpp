...
void getBounds(const Bodies &bodies, real_t &R0, real_t *X0)
{
                ...
                real_t *X, real_t R, int level = 0, bool direction = false)
{
  ...
  cell->level = level;            // Level of cell in tree
  //! If cell is a leaf
  ...
  cell->ChildOffset = child - &cells[0];                // Offset of first child cell
...
Cells buildTree(Bodies &bodies)
{
  ...
  for (auto &cell : cells) {
    cell.BodyOffset = cell.BODY - &bodies[0];
  }
